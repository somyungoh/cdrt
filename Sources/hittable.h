...
struct SHitRec
{
    ...
    glm::vec3                   n;
    float                       t;
    std::shared_ptr<IHittable>  p_hittable;
...
{
public:
    virtual bool    Hit(const CRay &ray, float t_min, float t_max, SHitRec &hitRec) = 0;
    virtual bool    HitAll(const CRay &ray, float t_min, float t_max, VHits &hits) = 0;
